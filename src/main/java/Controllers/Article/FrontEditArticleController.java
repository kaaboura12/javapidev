...
public class FrontEditArticleController implements Initializable {
    @FXML
    ...
    private void loadGaleries() {
        try {
            ...
            if (galeries != null && !galeries.isEmpty()) {
                galerieComboBox.getItems().clear();
                galerieComboBox.getItems().addAll(galeries);
                ...
                galerieComboBox.setButtonCell(new ListCell<Galerie>() {
                    @Override
                    protected void updateItem(Galerie item, boolean empty) {
                        super.updateItem(item, empty);
                        setText(empty ? "" : item.getNom());
        ...
        if (validateFields()) {
            try {
                if (article != null) {
                    ...
                    showError("Erreur : Article non défini pour la modification.");
                    return;
                }
    ...
    private void updateArticle() throws SQLException {
        if (article == null) {
            throw new SQLException("Tentative de mise à jour d'un article null.");
        }
        ...
        article.setGalerieId(galerieComboBox.getValue().getId());
        article.setNbrArticle(Integer.parseInt(nbrArticleField.getText()));
        article.setDisponible(disponibleCheckBox.isSelected());

        if (selectedImageFile != null) {
            String imagePath = copyImageToUploadFolder(selectedImageFile);
            if (imagePath != null) {
                article.setContenu(imagePath);
            } else {
                System.err.println("Erreur copie image pour article ID: " + article.getId());
            }
        }
    ...
    private boolean validateFields() {
        ...
        if (galerieComboBox.getValue() == null) {
