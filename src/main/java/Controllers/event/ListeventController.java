...
public class ListeventController extends BasefrontController {
    ...
    private void handleGetTicket(Event event) {
        try {
            ...
            if (event.getAvailableSeats() > 0) {
                // Get the content area for navigation
                ...
                if (contentArea != null) {
                    ...
                    // Fallback to opening in a new window if content area not found
    ...
    private void handleDonation(Event event) {
        try {
            // Check if parent content area exists
            StackPane contentArea = findContentArea();
            if (contentArea != null) {
            ...
            showErrorAlert("Error", "Failed to open donation form.", e.getMessage());
            e.printStackTrace();
        }
    }
